...
# the classes ADMstate and digester are defined in the plant.dll in the namespace biogas
from biogas import ADMstate, digester


def get_state_zero_from_initial_state(csv_file):
    """
    Read csv_file and return the initial ADM1 state vector as a list
    :param csv_file: file in which initial ADM1 state vector is saved
    :return:
    """
    ...
    state_zero = [S_su, S_aa, S_fa, S_va, S_bu, S_pro, S_ac, S_h2, S_ch4, S_co2, S_nh4_ion, S_I,
                  X_xc, X_ch, X_pr, X_li, X_su, X_aa, X_fa, X_c4, X_pro, X_ac, X_h2, X_I, X_p,
                  S_cation, S_anion, S_va_ion, S_bu_ion, S_pro_ion, S_ac_ion, S_hco3_ion,
                  S_nh3, pi_Sh2, pi_Sch4, pi_Sco2, pTOTAL]
...
class PyADM1:
    ...
    def __init__(self, feedstock):
        ...
        :param feedstock: object of the feedstock class
        """
    ...
    def createInfluent(self, Q, i):
        """
        Create ADM1 input stream from volumetric flow rate vector Q and set self._state_input so that the ADM1 knows
        the current input.

        :param Q: list of volumetric flow rates for each substrate. Length of list must be equal to number of
    ...
    def save_final_state_in_csv(self, simulate_results, filename="digester_initial6.csv"):
        """
        Save final ADM1 state vector into given filename to be the initial state for a future simulation
        :param simulate_results: list of ADM1 state vectors collected during simulations
        :param filename: file to save final state vector into
        """
        columns = [*self._feedstock.header()[:-1],
    ...
    def calc_gas(self, pi_Sh2, pi_Sch4, pi_Sco2, pTOTAL):
        """
        Calculate biogas production rates

        :param pi_Sh2: partial pressure of hydrogen
        :param pi_Sch4: partial pressure of methane
        :param pi_Sco2: partial pressure of carbon dioxide
        :param pTOTAL: total partial pressure
        :return:
        """
    ...
    def print_params_at_current_state(self, state_ADM1xp):
        """
        Calculates a couple of process values from given state vector and stores them into lists
    ...
    def ADM1_ODE(self, t, state_zero):
        ...
        :param t: model is not time-dependent
        :param state_zero: initial ADM1 state at which simulation is started
        :return: dx/dt
        """
        (N_xc, N_I, N_aa, C_xc, C_sI, C_ch, C_pr, C_li, C_xI, C_su, C_aa, f_fa_li, C_fa,
        ...
        K_H_co2, K_H_ch4, K_H_h2) = ADMparams.getADMparams(self._R, self._T_base, self._T_ad)

        (f_ch_xc, f_pr_xc, f_li_xc, f_xI_xc, f_sI_xc, f_xp_xc, k_dis, k_hyd_ch, k_hyd_pr, k_hyd_li,
        k_m_c4, k_m_pro, k_m_ac, k_m_h2) = self._get_substrate_dependent_params()
        ...
        diff_S_bu = (q_ad / self.V_liq * (S_bu_in - S_bu) + (1 - Y_su) * f_bu_su * Rho_5 +
                   (1 - Y_aa) * f_bu_aa * Rho_6 - Rho_9)  # eq5                             OK

        diff_S_pro = (q_ad / self.V_liq * (S_pro_in - S_pro) + (1 - Y_su) * f_pro_su * Rho_5 +
                    (1 - Y_aa) * f_pro_aa * Rho_6 +
                    (1 - Y_c4) * 0.54 * Rho_8 - Rho_10)  # eq6                              OK, factor 0.54 = fpro_va

        diff_S_ac = (q_ad / self.V_liq * (S_ac_in - S_ac) + (1 - Y_su) * f_ac_su * Rho_5 +
                   (1 - Y_aa) * f_ac_aa * Rho_6 +
                   (1 - Y_fa) * 0.7 * Rho_7 + (1 - Y_c4) * 0.31 * Rho_8 + (1 - Y_c4) * 0.8 * Rho_9 +
                   (1 - Y_pro) * 0.57 * Rho_10 - Rho_11)  # eq7         OK factors are fac_fa, fac_va, fac_bu, fac_pro

        diff_S_h2 = (q_ad / self.V_liq * (S_h2_in - S_h2) + (1 - Y_su) * f_h2_su * Rho_5 +
                   (1 - Y_aa) * f_h2_aa * Rho_6 +
                   (1 - Y_fa) * 0.3 * Rho_7 + (1 - Y_c4) * 0.15 * Rho_8 + (1 - Y_c4) * 0.2 * Rho_9 +
                   (1 - Y_pro) * 0.43 * Rho_10 - Rho_12 - self._V_gas / self.V_liq * Rho_T_8)

        # eq9
        diff_S_ch4 = (q_ad / self.V_liq * (S_ch4_in - S_ch4) + (1 - Y_ac) * Rho_11 +
                    (1 - Y_h2) * Rho_12 - self._V_gas / self.V_liq * Rho_T_9)

        ## eq10 start##
        s_1 =  (-1 * C_xc + f_sI_xc * C_sI + f_ch_xc * C_ch + f_pr_xc * C_pr +
              f_li_xc * C_li + f_xI_xc * C_xI)          # OK, equals -fco2Xc except that in C# we also have Xp
        ...
        s_5 =  (-1 * C_su + (1 - Y_su) * (f_bu_su * C_bu + f_pro_su * C_pro + f_ac_su * C_ac) +
              Y_su * C_bac)         # OK, equals -fco2,su
        s_6 =  (-1 * C_aa + (1 - Y_aa) * (f_va_aa * C_va + f_bu_aa * C_bu + f_pro_aa * C_pro +
                                        f_ac_aa * C_ac) + Y_aa * C_bac)         # OK, equals -fco2,aa
        ...
        Sigma =  (s_1 * Rho_1 + s_2 * Rho_2 + s_3 * Rho_3 + s_4 * Rho_4 + s_5 * Rho_5 + s_6 * Rho_6 + s_7 * Rho_7 +
                s_8 * Rho_8 + s_9 * Rho_9 + s_10 * Rho_10 + s_11 * Rho_11 + s_12 * Rho_12 +
                s_13 * (Rho_13 + Rho_14 + Rho_15 + Rho_16 + Rho_17 + Rho_18 + Rho_19))          # OK, except s_2, s_3
        ...
        diff_S_nh4_ion = (q_ad / self.V_liq * (S_nh4_in - S_nh4_ion) - #+ (N_xc - f_xI_xc * N_I - f_sI_xc * N_I-f_pr_xc * N_aa) * Rho_1 -
                   ...
                   (N_bac - N_xc) * (Rho_13 + Rho_14 + Rho_15 + Rho_16 + Rho_17 + Rho_18 + Rho_19)) + Rho_A_11 # eq11
        ...
        diff_X_ch = (q_ad / self.V_liq * (X_ch_in - X_ch) + f_ch_xc * Rho_1 - Rho_2 + f_ch_xb * (Rho_13 + Rho_14 + Rho_15 +
                   Rho_16 + Rho_17 + Rho_18 + Rho_19)) # eq14

        diff_X_pr = (q_ad / self.V_liq * (X_pr_in - X_pr) + f_pr_xc * Rho_1 - Rho_3 + f_pr_xb * (Rho_13 + Rho_14 + Rho_15 +
                   Rho_16 + Rho_17 + Rho_18 + Rho_19)) # eq15

        diff_X_li = (q_ad / self.V_liq * (X_li_in - X_li) + f_li_xc * Rho_1 - Rho_4 + f_li_xb * (Rho_13 + Rho_14 + Rho_15 +
                   Rho_16 + Rho_17 + Rho_18 + Rho_19)) # eq16
        ...
        diff_X_p = (q_ad / self.V_liq * (X_p_in - X_p) - f_xp_xc * Rho_1 + f_p * (Rho_13 + Rho_14 + Rho_15 +
                   Rho_16 + Rho_17 + Rho_18 + Rho_19))
        ...
        return (diff_S_su, diff_S_aa, diff_S_fa, diff_S_va, diff_S_bu, diff_S_pro, diff_S_ac, diff_S_h2, diff_S_ch4, diff_S_co2,
              diff_S_nh4_ion, diff_S_I, diff_X_xc, diff_X_ch, diff_X_pr, diff_X_li, diff_X_su, diff_X_aa, diff_X_fa, diff_X_c4,
              diff_X_pro, diff_X_ac, diff_X_h2, diff_X_I, diff_X_p, diff_S_cation, diff_S_anion, diff_S_va_ion,
              diff_S_bu_ion, diff_S_pro_ion, diff_S_ac_ion, diff_S_hco3_ion, diff_S_nh3,
              diff_p_gas_h2, diff_p_gas_ch4, diff_p_gas_co2, diff_pTOTAL)

    # *** PRIVATE methods ***

    def _setInfluent(self, influent_state, i):
        """
        Function to set influent values for ADM1 influent state variables at each simulation step. Sets the
        self._state_input variable that is accessed by the ADM1_ODE method.

        :param influent_state: dictionary containing the input variables of the ADM1 values over n time steps
        :param i: the ith value is taken out of the given influent_state dictionary.
        """
        ##variable definition
        ...
        self._state_input = [S_su_in, S_aa_in, S_fa_in, S_va_in, S_bu_in, S_pro_in, S_ac_in, S_h2_in, S_ch4_in,
                           S_IC_in, S_IN_in, S_I_in, X_xc_in, X_ch_in, X_pr_in, X_li_in, X_su_in, X_aa_in, X_fa_in,
                             X_c4_in, X_pro_in, X_ac_in, X_h2_in,
                           X_I_in, X_p_in, S_cation_in, S_anion_in, S_va_ion_in, S_bu_ion_in, S_pro_ion_in,
                           S_ac_ion_in, S_hco3_ion_in, S_nh3_in, q_ad]

    def _get_substrate_dependent_params(self):
        """
        Returns the substrate dependent ADM1 parameters. Calculates those parameters via C# DLL methods. The parameters
        ...
        return (f_ch_xc, f_pr_xc, f_li_xc, f_xI_xc, f_sI_xc, f_xp_xc, k_dis, k_hyd_ch, k_hyd_pr, k_hyd_li,
              k_m_c4, k_m_pro, k_m_ac, k_m_h2)

    def _calc_gas(self, state_ADM1):
        """
        Calculates biogas, methane, co2 production rates from given ADM1 state vector
        :param state_ADM1: ADM1 state vector as 37 dimensional vector
        :return:
        """
